"""
Normalize Twitter capture data into AlgorithmLens UnifiedScanResult format.

This bridges the gap between our common capture schema and the
existing analysis pipeline that expects UnifiedScanResult dicts.
"""
...
def _map_content_type(capture_type: str) -> str:
    """Map our common content_type to UnifiedScanResult content_type values."""
    mapping = {
        "text": "TEXT",
        "image": "IMAGE",
        "video": "VIDEO",
    ...
    return mapping.get(capture_type.lower(), "TEXT")
...
def _post_to_feed_item(post: CapturedPost, index: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "ad_metadata": {
            "ad_detected_reason": "promoted_label" if post.is_ad else None,
            "sponsored_label_text": "Promoted" if post.is_ad else None,
        ...
        "topics": {
            "primary_category": None,  # Will be filled by Gemini
            "secondary_categories": [],
            "freeform_tags": [],
        },
        "political": {
            "is_political": False,  # Will be filled by Gemini
        ...
        "source_details": {
            ...
            "dom_metadata": {
                "post_id": post.id,
                "post_url": f"https://x.com/{post.author}/status/{post.id}" if post.id and not post.id.startswith("tweet_") else None,
                "account_id": None,
            },
            "ocr_metadata": None,
        },
...
def normalize_to_unified_scan(snapshot: CaptureSnapshot) -> Dict[str, Any]:
    ...
    This produces the exact format expected by the AlgorithmLens
    analysis pipeline (gemini_analyzer.py, evidence bundle builders).
    """
    ...
    # Build content type distribution for aggregates
    ...
    return {
        ...
        "aggregates": {
            ...
            "topic_distribution": [],  # Will be computed by Gemini analysis
            "wellbeing_summary": {
        ...
        # Extra field for eval tracking (not part of standard schema)
        "_eval_metadata": {
