"""
Normalize TikTok capture data into AlgorithmLens UnifiedScanResult format.
"""
...
def _map_content_type(capture_type: str) -> str:
    """Map TikTok content_type to UnifiedScanResult content_type values."""
    # TikTok is essentially all video
    return "VIDEO"
...
def _post_to_feed_item(post: CapturedPost, index: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "source_details": {
            ...
            "dom_metadata": {
                "post_id": post.id,
                "post_url": f"https://www.tiktok.com/@{post.author}/video/{post.id}" if post.id and not post.id.startswith("tt_") else None,
                "account_id": None,
            },
            "ocr_metadata": None,
        },
