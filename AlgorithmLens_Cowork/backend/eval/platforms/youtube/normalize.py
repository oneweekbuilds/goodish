"""
Normalize YouTube capture data into AlgorithmLens UnifiedScanResult format.
"""
...
def _map_content_type(capture_type: str) -> str:
    """Map YouTube content_type to UnifiedScanResult content_type values."""
    mapping = {
        "video": "VIDEO",
        "short": "VIDEO",
        "live": "VIDEO",
    ...
    return mapping.get(capture_type.lower(), "VIDEO")
...
def _post_to_feed_item(post: CapturedPost, index: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "ad_metadata": {
            "ad_detected_reason": "ad_badge" if post.is_ad else None,
            "sponsored_label_text": "Ad" if post.is_ad else None,
        ...
        "source_details": {
            ...
            "dom_metadata": {
                "post_id": post.id,
                "post_url": f"https://www.youtube.com/watch?v={post.id}" if post.id and not post.id.startswith("yt_") else None,
                "account_id": None,
            },
            "ocr_metadata": None,
        },
