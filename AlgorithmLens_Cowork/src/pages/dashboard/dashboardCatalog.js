...
export const dashboardCatalog = [
  ...
  // --- PRIMARY: The single most important ad metric ---
  {
    ...
    description: 'Shows what share of your feed contains labeled ads and likely promotional content.',
  ...
  },
  {
    ...
    action: () => 'You can choose to engage less with promotional content you would rather not see.',
  },
  ...
  },
  {
    ...
    action: () => 'Mute accounts whose tone you find unhelpful.',
  },
  {
    tab: 'politics',
    id: 'politics-trend',
  ...
  // =====================================================
  {
    ...
    id: 'suggested-split',
    title: 'Your feed split',
    description: 'How much of your feed comes from suggested posts vs accounts you follow.',
    outputType: 'stacked100',
    dataFn: 'getSuggestedVsFollowedPercentData',
    isPrimary: true,
    sortOrder: 'primary',
    takeaway: (data) => {
    ...
    action: () => 'Following more accounts in topics you care about can help balance what you see.',
    whyExplanation: 'Source origin metadata from the platform indicates whether each post was algorithmically suggested or from an account you follow.',
    counterfactual: 'This is only as accurate as the platform\'s source origin metadata.',
  },
  {
    tab: 'suggested_vs_followed',
  ...
  },
  {
    ...
    sortOrder: 'supporting',
    takeaway: (data) => {
      const delta = data?.adDelta || 0;
      if (Math.abs(delta) < 3) return 'Similar levels of commercial content in suggested and followed posts.';
  ...
  },
  {
    ...
    sortOrder: 'supporting',
    collapsedByDefault: true,
    takeaway: () => 'Compares emotional tone between suggested and followed content.',
    whyExplanation: 'Groups each post\'s sentiment valence by source origin.',
  },
  {
    tab: 'suggested_vs_followed',
    ...
    collapsedByDefault: true,
    takeaway: (data) => {
      const delta = data?.dominantDelta;
      return delta ? delta.insight : 'Similar content formats in suggested and followed posts.';
    },
    whyExplanation: 'Compares the content_type field (VIDEO, IMAGE, etc.) between suggested and followed posts.',
  },
...
export const getViewsForTab = (tabId, options = {}) => {
  const { includeHidden = false, isPlusUser = false } = options;

  return dashboardCatalog
    .filter((view) => {
      // Must match tab
      if (view.tab !== tabId) return false;
      // Premium-only views: show for Plus users, hide for free users
      if (view.premiumOnly) {
        return isPlusUser || includeHidden;
      }
