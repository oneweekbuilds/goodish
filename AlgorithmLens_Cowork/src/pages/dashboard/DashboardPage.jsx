...
import { Loader2, RefreshCw, BarChart3, Clock, Globe, Database, ChevronDown, ChevronUp, Compass, RefreshCcw, Lock, Sparkles, ExternalLink, ShieldCheck, MessageSquare, EyeOff, CheckCircle, X, Megaphone, Scale } from 'lucide-react';
...
const TabHero = ({
  ...
  if (hasHeroData && heroQualityOk && typeof heroView?.takeaway === 'function') {
    try {
      ...
      logError('DashboardPage', `Error computing hero takeaway for ${heroView?.id}:`, err);
      headline = null;
    }
  }
...
const DashboardPage = () => {
  ...
  // Handle arrow key navigation for tabs (WCAG keyboard support)
  ...
  useEffect(() => {
    if (userTier === 'free') {
      ...
      if (!allowedPresets.includes(datePreset)) {
        // Reset to 'all' if current preset is not allowed for free tier
        setDatePreset('all');
  ...
  // Only UPGRADES tier (never downgrades from Plus if backend is out of sync)
  useEffect(() => {
    ...
    const sync = async () => {
      try {
        ...
        // Don't downgrade: if backend says free but localStorage says plus,
        // the backend may not have received the Stripe webhook yet.
        // The verify-checkout endpoint or webhook will eventually sync.
      } catch (err) {
        // Fail silently — keep current tier from localStorage
        logWarning('DashboardPage', '[DashboardPage] Entitlements sync failed:', err);
      }
    };
    sync();
    return () => { cancelled = true; };
  }, [authReady, isDemoMode]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Auto-select most-used platform for free users (single-platform restriction)
  useEffect(() => {
    if (userTier !== 'free' || !scans || scans.length === 0) return;
    // Count scans per platform
  ...
  useEffect(() => {
    ...
    // Mark as synced only after we know auth is ready and we'll actually run
    checkoutSyncedRef.current = true;

    // Optimistic activation: Stripe already confirmed payment succeeded
    // (the redirect to ?checkout=success only happens after successful payment).
    // Activate Plus immediately so the user sees the unlocked UI right away.
    setStoredPlanTier(PLAN_TIERS.PLUS);
    setPlanTier(PLAN_TIERS.PLUS);
    ...
    const syncEntitlements = async () => {
      try {
        // Verify with backend — create subscription record if webhook was missed
        ...
        try {
          ...
          if (verifyResponse.ok) {
            ...
            if (verifyData.is_plus) {
              return; // Backend confirmed — all good
            }
          }
        } catch (networkErr) {
          // Backend unreachable — that's OK, we already activated optimistically.
          // The backend will catch up when webhook arrives or on next page load.
          logWarning('DashboardPage', '[checkout] Backend unreachable for verify-checkout, using optimistic activation:', networkErr);
          return;
        }

        // verify-checkout didn't confirm Plus — poll entitlements as fallback
        ...
        for (let attempt = 0; attempt < delays.length; attempt++) {
          ...
          try {
            ...
            // Network error during poll — keep optimistic activation
            return;
          }
        }

        // Backend says not Plus after all attempts — but Stripe DID succeed.
        // Keep the optimistic activation. The webhook will eventually arrive
        // and sync the backend. Don't downgrade the user's experience.
        logWarning('DashboardPage', '[checkout] Backend could not confirm Plus yet, keeping optimistic activation');
    ...
    for (const view of currentViews) {
      ...
      if (typeof dataFn === 'function') {
        try {
          ...
          logError('DashboardPage', `Error computing data for ${view.id}:`, err, err.stack);
  ...
  if (error && !isDemoMode) {
    ...
    // For other errors, show a calm error message
    return (
      <div className="min-h-screen bg-bg-page pt-24 md:pt-28 pb-16 px-4 md:px-6">
        <div className="max-w-2xl mx-auto px-2 sm:px-4 md:px-6">
      ...
      <div className="min-h-screen bg-bg-page pt-24 md:pt-28 pb-16 px-4 md:px-6">
        <div className="max-w-2xl mx-auto px-2 sm:px-4 md:px-6">
          <div className="bg-white rounded-2xl shadow-md border border-slate-100 p-6 sm:p-8 md:p-12 text-center">
            ...
            <h1 className="text-2xl font-bold text-text-main mb-2">Your Dashboard</h1>
            <p className="text-text-muted mb-8 max-w-md mx-auto">
              You'll see your feed analysis here after your first scan. Scan any platform to get started.
            </p>
            <Link
              to="/start"
  ...
  return (
    <>
      ...
      {/* Post-first-scan onboarding walkthrough */}
      <ScanWalkthrough />
      ...
      <div className="max-w-7xl mx-auto px-0 sm:px-2 md:px-6">
          ...
          <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <button
                onClick={async () => {
                  try {
                    // Try verify-checkout first (creates subscription if webhook was missed)
                    const apiBase = getApiBaseUrl();
                    const verifyResponse = await authenticatedFetch(`${apiBase}/api/stripe/verify-checkout`, {
                      method: 'POST',
                    });
                    ...
                    logError('DashboardPage', 'Refresh failed:', err);
                  }
                }}
        ...
        {/* B6: Payment failed banner for past_due subscriptions */}
        <PastDueBanner />
        ...
        {isPremiumUser ? (
          ...
          /* Free tier: show current platform with upgrade hint */
          <div className="mb-6 p-4 rounded-xl border border-slate-200 bg-white">
        ...
        {/* Tab Navigation - PREMIER QUALITY pill-style tabs */}
        <div className={`${isOnAlgorithmTab ? 'mb-6' : 'mb-10'} overflow-hidden`}>
          ...
          >
            {TABS.map((tab) => {
              ...
              // Tab icon mapping
              const tabIcons = {
                overview: <BarChart3 size={15} className="flex-shrink-0" />,
                sources: <Globe size={15} className="flex-shrink-0" />,
                ads: <Megaphone size={15} className="flex-shrink-0" />,
                politics: <Scale size={15} className="flex-shrink-0" />,
                tone: <MessageSquare size={15} className="flex-shrink-0" />,
                suggested_vs_followed: <Compass size={15} className="flex-shrink-0" />,
              };
                ...
                <button
                  ...
                  style={{
                    ...
                    background: isActive
                      ...
                      : 'transparent',
                    boxShadow: isActive
                      ? `0 2px 8px ${accent}30, 0 1px 3px rgba(0,0,0,0.06)`
                      : 'none',
