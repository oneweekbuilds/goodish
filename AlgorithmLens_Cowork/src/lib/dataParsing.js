 ...
 * @param {Object} scanData - Extracted scan data (optional, defaults to data)
 * @returns {string} - 'desktop' | 'mobile'
 */
export function detectScanSource(data, scanData = null) {
  // If scanData not provided, extract it from data
  if (!scanData) {
    scanData = data.result || data.scan || data;
  }
