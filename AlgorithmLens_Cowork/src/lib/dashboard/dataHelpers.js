...
import {
  ...
  // Algorithm's Picks (Suggested vs Followed) additions
 ...
 */
export function getFeedItems(scanDetail) {
...
export function getAlgoUncertainData(scans, scanDetails) {
  ...
  if (stability.overlapPercent < 50) {
    insights.push('Your topics vary significantly between scans, suggesting the feed composition may shift as content patterns change.');
  } else {
    insights.push('Your feed appears relatively stable, suggesting consistent patterns in what content is being shown.');
  }
...
/**
 * View: Overall suggested vs followed split
 * Wraps aggregateSourceOrigin with standard envelope
 */
export function getSuggestedVsFollowedPercentData(scans, scanDetails) {
  ...
  if (!sourceData.hasData) {
    return createResponse(
      false,
      null,
      'Need source origin metadata to analyze how algorithmic recommendations shape your feed.',
      0,
      []
    );
  }
  ...
  return createResponse(
    true,
    {
      suggestedPercentage: sourceData.suggestedPercentage,
      followedPercentage: sourceData.followedPercentage,
      totalSuggested: sourceData.totalSuggested,
      totalFollowed: sourceData.totalFollowed,
      totalPosts: sourceData.totalPosts,
      byPlatform: sourceData.byPlatform,
    },
    null,
    sourceData.scansUsed,
    sourceData.scansWithData
  );
}

/**
 * View: Topics comparison — what the algorithm suggests vs what you follow
 * Minimum: 30 total posts across both groups
 */
export function getSuggestedTopicsData(scans, scanDetails) {
  const topicsData = aggregateTopicsBySourceOrigin(scans, scanDetails);

  if (!topicsData.hasData) {
    return createResponse(
      false,
      null,
      'Need more posts with topic data in both suggested and followed groups.',
      topicsData.scansUsed,
      topicsData.scansWithData
    );
  }

  // Total threshold: 30 posts
  if (topicsData.suggestedTotal + topicsData.followedTotal < 30) {
    return createResponse(
      false,
      null,
      'Need at least 30 posts with topic data to compare suggested vs followed topics.',
      topicsData.scansUsed,
      topicsData.scansWithData
    );
  }
  ...
  return createResponse(
    true,
    {
      topAlgorithmTopics: topicsData.topAlgorithmTopics,
      topFollowedTopics: topicsData.topFollowedTopics,
      suggestedTotal: topicsData.suggestedTotal,
      followedTotal: topicsData.followedTotal,
    },
    null,
    topicsData.scansUsed,
    topicsData.scansWithData
...
export function getSuggestedAdsData(scans, scanDetails) {
  const adsData = aggregateAdsBySourceOrigin(scans, scanDetails);

  if (!adsData.hasData) {
    return createResponse(
      false,
      null,
      'Need more posts in both suggested and followed groups to compare commercial content.',
      adsData.scansUsed,
      adsData.scansWithData
    );
  }

  if (adsData.suggestedTotal < 20 || adsData.followedTotal < 20) {
    return createResponse(
      false,
      null,
      'Need at least 20 posts in each group to compare commercial content.',
      adsData.scansUsed,
      adsData.scansWithData
    );
  }
  ...
  return createResponse(
    true,
    {
      suggestedAdPercent: adsData.suggestedAdPercent,
      followedAdPercent: adsData.followedAdPercent,
      adDelta: adsData.adDelta,
      deltaInsight: adsData.deltaInsight,
      suggestedTotal: adsData.suggestedTotal,
      followedTotal: adsData.followedTotal,
    },
    null,
    adsData.scansUsed,
    adsData.scansWithData
...
/**
 * View: Creator familiarity — does the algorithm show new or familiar creators?
 * Minimum: 5 creators in each group
 */
export function getCreatorFamiliarityData(scans, scanDetails) {
  const familiarityData = aggregateCreatorFamiliarityBySourceOrigin(scans, scanDetails);

  if (!familiarityData.hasData) {
    return createResponse(
      false,
      null,
      'Need more creators in both suggested and followed groups to compare.',
      familiarityData.scansUsed,
      familiarityData.scansWithData
    );
  }
  ...
  return createResponse(
    true,
    {
      suggestedCreatorCount: familiarityData.suggestedCreatorCount,
      followedCreatorCount: familiarityData.followedCreatorCount,
      overlapCount: familiarityData.overlapCount,
      noveltyPercent: familiarityData.noveltyPercent,
      familiarPercent: familiarityData.familiarPercent,
    },
    null,
    familiarityData.scansUsed,
    familiarityData.scansWithData
...
export function getContentTypeByOriginData(scans, scanDetails) {
  const contentData = aggregateContentTypeBySourceOrigin(scans, scanDetails);

  if (!contentData.hasData) {
    return createResponse(
      false,
      null,
      'Need more posts in both groups to compare content formats.',
      contentData.scansUsed,
      contentData.scansWithData
    );
  }

  if (contentData.suggestedTotal < 20 || contentData.followedTotal < 20) {
    return createResponse(
      false,
      null,
      'Need at least 20 posts in each group to compare content formats.',
      contentData.scansUsed,
      contentData.scansWithData
    );
  }
  ...
  return createResponse(
    true,
    {
      suggested: contentData.suggested,
      followed: contentData.followed,
      suggestedTotal: contentData.suggestedTotal,
      followedTotal: contentData.followedTotal,
      dominantDelta: contentData.dominantDelta,
    },
    null,
    contentData.scansUsed,
    contentData.scansWithData
