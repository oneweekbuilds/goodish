...
export function useAdsEvidenceBundle(
  ...
  const fetchBundle = useCallback(async () => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/evidence-bundle/ads${debugParam}`);
      ...
      setError(message);
      logError('useEvidenceBundle', '[Evidence Bundle] Error fetching:', err);
    } finally {
      setLoading(false);
    }
  }, [scanId, enabled]);
...
export function useAdsTalkToAlgorithm(scanId: string | null): UseTalkToAlgorithmResult {
  ...
  const sendQuestion = useCallback(async (question: string): Promise<TalkResponse | null> => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/talk/ads`, {
        method: 'POST',
        body: formData,
      });
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Talk to Algorithm] Error:', err);
      return null;
    } finally {
      setLoading(false);
    }
...
export function usePoliticsEvidenceBundle(
  ...
  const fetchBundle = useCallback(async () => {
    ...
    try {
      // Include debug info in dev mode
      const debugParam = IS_DEV ? '?debug=true' : '';
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/evidence-bundle/politics${debugParam}`);
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Evidence Bundle] Error fetching politics bundle:', err);
    } finally {
      setLoading(false);
    }
  }, [scanId, enabled]);
...
export function usePoliticsTalkToAlgorithm(scanId: string | null): UsePoliticsTalkToAlgorithmResult {
  ...
  const sendQuestion = useCallback(async (question: string): Promise<PoliticsTalkResponse | null> => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/talk/politics`, {
        method: 'POST',
        body: formData,
      });
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Talk to Algorithm] Politics Error:', err);
      return null;
    } finally {
      setLoading(false);
    }
...
export function usePatternsEvidenceBundle(
  ...
  const fetchBundle = useCallback(async () => {
    ...
    try {
      // Include debug info in dev mode
      const debugParam = IS_DEV ? '?debug=true' : '';
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/evidence-bundle/patterns${debugParam}`);
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Evidence Bundle] Error fetching patterns bundle:', err);
    } finally {
      setLoading(false);
    }
  }, [scanId, enabled]);
...
export function usePatternsTalkToAlgorithm(scanId: string | null): UsePatternsTalkToAlgorithmResult {
  ...
  const sendQuestion = useCallback(async (question: string): Promise<PatternsTalkResponse | null> => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/talk/patterns`, {
        method: 'POST',
        body: formData,
      });
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Talk to Algorithm] Patterns Error:', err);
      return null;
    } finally {
      setLoading(false);
    }
...
export function useCreatorsEvidenceBundle(
  ...
  const fetchBundle = useCallback(async () => {
    ...
    try {
      // Include debug info in dev mode
      const debugParam = IS_DEV ? '?debug=true' : '';
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/evidence-bundle/creators${debugParam}`);
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Evidence Bundle] Error fetching creators bundle:', err);
    } finally {
      setLoading(false);
    }
  }, [scanId, enabled]);
...
export function useCreatorsTalkToAlgorithm(scanId: string | null): UseCreatorsTalkToAlgorithmResult {
  ...
  const sendQuestion = useCallback(async (question: string): Promise<CreatorsTalkResponse | null> => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/talk/creators`, {
        method: 'POST',
        body: formData,
      });
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Talk to Algorithm] Creators Error:', err);
      return null;
    } finally {
      setLoading(false);
    }
...
export function useInferencesEvidenceBundle(
  ...
  const fetchBundle = useCallback(async () => {
    ...
    try {
      // Include debug info in dev mode
      const debugParam = IS_DEV ? '?debug=true' : '';
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/evidence-bundle/inferences${debugParam}`);
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Evidence Bundle] Error fetching inferences bundle:', err);
    } finally {
      setLoading(false);
    }
  }, [scanId, enabled]);
...
export function useInferencesTalkToAlgorithm(scanId: string | null): UseInferencesTalkToAlgorithmResult {
  ...
  const sendQuestion = useCallback(async (question: string): Promise<InferencesTalkResponse | null> => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE}/scans/${scanId}/talk/inferences`, {
        method: 'POST',
        body: formData,
      });
      ...
      const message = err instanceof Error ? err.message : 'Unknown error';
      setError(message);
      logError('useEvidenceBundle', '[Talk to Algorithm] Inferences Error:', err);
      return null;
    } finally {
      setLoading(false);
    }
