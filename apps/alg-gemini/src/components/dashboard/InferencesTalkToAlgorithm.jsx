...
import { useInferencesTalkToAlgorithm, useInferencesEvidenceBundle } from '../../hooks/useEvidenceBundle';
import EvidenceSummaryBadge, { computeEvidenceSummary, getEvidenceAwarePrompts } from './EvidenceSummaryBadge';

/**
 * InferencesTalkToAlgorithm - Evidence-bound Talk to Your Algorithm for "What the Algorithm Thinks" tab
 *
...
// Default prompts for Inferences / What the Algorithm Thinks (used when coverage is good)
const DEFAULT_INFERENCES_PROMPTS = [
...
const PremiumInvitationCard = ({ onStartConversation, onSelectPrompt, bundle }) => {
  ...
  const prompts = useMemo(
    () => getEvidenceAwarePrompts('algorithm', evidenceSummary, DEFAULT_INFERENCES_PROMPTS),
    [evidenceSummary]
  );
    ...
    >
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6 relative">
        <div className="flex-1 min-w-0">
          ...
          >
            Explore signals detected in your content and their confidence levels.
          </p>
        </div>
      ...
      >
        Ask questions about algorithmic signals and inferences. Responses are generated strictly from your scan data—no guessing, no generic explanations.
      </p>
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          >
            Talk to Your Algorithm - What It Thinks
          </p>
        </div>
        <span
      ...
      >
        <div className="flex items-center gap-4">
          <input
            ...
            placeholder="Ask about algorithmic signals..."
...
/**
 * Main Component - InferencesTalkToAlgorithm
 */
const InferencesTalkToAlgorithm = ({ scanId }) => {
  ...
  const { sendQuestion, loading: talkLoading, error: talkError } = useInferencesTalkToAlgorithm(scanId);
  const { bundle } = useInferencesEvidenceBundle(scanId);
