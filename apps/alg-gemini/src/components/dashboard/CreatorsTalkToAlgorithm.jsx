...
import { useCreatorsTalkToAlgorithm, useCreatorsEvidenceBundle } from '../../hooks/useEvidenceBundle';
import EvidenceSummaryBadge, { computeEvidenceSummary, getEvidenceAwarePrompts } from './EvidenceSummaryBadge';

/**
 * CreatorsTalkToAlgorithm - Evidence-bound Talk to Your Algorithm for Creators & Voices tab
 *
...
// Default prompts for Creators & Voices (used when coverage is good)
const DEFAULT_CREATORS_PROMPTS = [
...
const PremiumInvitationCard = ({ onStartConversation, onSelectPrompt, bundle }) => {
  ...
  const prompts = useMemo(
    () => getEvidenceAwarePrompts('creators', evidenceSummary, DEFAULT_CREATORS_PROMPTS),
    [evidenceSummary]
  );
    ...
    >
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6 relative">
        <div className="flex-1 min-w-0">
          ...
          >
            Explore which creators and voices appeared in your feed.
          </p>
        </div>
      ...
      >
        Ask questions about the creators in your feed. Responses are generated strictly from your scan data—no guessing, no generic explanations.
      </p>
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          >
            Talk to Your Algorithm - Creators & Voices
          </p>
        </div>
        <span
      ...
      >
        <div className="flex items-center gap-4">
          <input
            ...
            placeholder="Ask about creators in your feed..."
...
/**
 * Main Component - CreatorsTalkToAlgorithm
 */
const CreatorsTalkToAlgorithm = ({ scanId }) => {
  ...
  const { sendQuestion, loading: talkLoading, error: talkError } = useCreatorsTalkToAlgorithm(scanId);
  const { bundle } = useCreatorsEvidenceBundle(scanId);
