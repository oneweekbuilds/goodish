...
def _detect_ai_disclosure(text: str) -> tuple:
    """
    ...
    # Platform AI disclosure keywords (strict exact phrases only)
    # Ordered by specificity (most specific first) to capture the best match
    ai_label_keywords = [
        # Instagram/Meta family (specific phrases)
        "made with ai",
        "imagined with ai",
        "created with ai",
        ...
        "altered content",
        "synthetic content",
        # Generic platform disclosures (specific multi-word phrases only)
        "ai content",
        "generated with ai",
    ]
...
def process_video(file_path: str, user_id: str = "demo-user", platform: str = "tiktok") -> UnifiedScanResult:
    ...
    while True:
        ...
        if current_frame % frame_interval == 0:
            ...
            ai_disclosure_label, ai_disclosure_matched_text = _detect_ai_disclosure(text)
