...
def _detect_ai_disclosure(text: str) -> str:
    """
    ...
    Returns:
        "LABELED_AI" if platform AI disclosure label detected
        "NOT_LABELED" if no label detected
    """
    normalized = _normalize_text_for_matching(text)
    if not normalized:
        return "NOT_LABELED"

    # Platform AI disclosure keywords (explicit labels only, not inference terms)
    ai_label_keywords = [
...
def _detect_c2pa_disclosure(text: str) -> str:
    """
    ...
    Note: C2PA is typically embedded in image/video metadata, but platforms may
    display on-screen indicators like "cr:" badge, "Content Credentials", etc.

    Returns:
        "HAS_C2PA" if C2PA indicator detected in on-screen text
        "NO_C2PA" if no indicator detected
    """
    normalized = _normalize_text_for_matching(text)
    if not normalized:
        return "NO_C2PA"

    # C2PA indicator keywords (visible badges or labels)
    c2pa_keywords = [
        "c2pa",
        "content credentials",
        "content credential",
...
def process_video(file_path: str, user_id: str = "demo-user", platform: str = "tiktok") -> UnifiedScanResult:
    ...
    while True:
        ...
        if current_frame % frame_interval == 0:
            ...
            ai_disclosure = _detect_ai_disclosure(text)
            item.ai_disclosure = ai_disclosure

            # C2PA / Content Credentials Detection (from OCR text)
            c2pa_disclosure = _detect_c2pa_disclosure(text)
            item.c2pa_disclosure = c2pa_disclosure
